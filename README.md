...
option_model/
│
├── option_model.ipynb          # Main model training & prediction notebook
├── option_model_analysis.ipynb # Backtesting analysis (runs after main model)
...
1. **option_model.ipynb** - Run this first to:
   - Collect market data via Polygon.io API
   - Engineer features and train XGBoost model
   - Generate predictions and save results to `option_model_results.csv`

2. **option_model_analysis.ipynb** - Run this second to:
   - Load results from the main model
   - Perform backtesting analysis with different strategies
   - Generate performance visualizations and equity curves

> **Note**: The analysis notebook depends on the CSV file generated by the main model notebook.
